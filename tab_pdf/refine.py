...
2. 가사 재배치 — `{"op":"lyric","measure":M,"beat":B,"text":"음절"}`
   ...
   음절 하나를 원래 자리에서 **2 beat 넘게** 옮기는 재배치도 폐기된다.
3. 코드명 — `{"op":"chord","measure":M,"beat":B,"name":"Am7"}`
   **이름은 `chord_row`·`chord_in_effect` 에서 가져온다.** 추출기가 코드 행을
   ...
   이름은 그 beat **위치의 표기**여야 한다 — beat 의 x 왼쪽에서 가장 가까운
   `chord_row` 토큰(없으면 `chord_in_effect`)만 받는다. 자리 바꾸기는 폐기된다.
...
4. 보이싱 — `{"op":"voicing","name":"Am7","frets":[0,1,0,2,0,-1]}`
   ...
   프렛이 실제로 그 코드대로 소리 나는지 음정으로 검증한다. 코드에 없는 음이
   섞이거나, **이름이 주장하는 음이 빠지면** 폐기된다 — Bm7 이라 이름 붙이려면
   7도(A)가 있어야 한다. 생략해도 되는 것은 완전5도, 그리고 11·13화음에서
   실제로 빼는 음뿐이다. 분수 코드는 지정된 베이스가 최저음이어야 한다.
