...
| 마디별 표기법 | fret 32, slash 26 (mixed·empty 없음) |
| beat | **497** |
| 총 노트 | **1560** (프렛 294 + 코드 보이싱 1266) |
| 스트로크 | **9** (PDF 원본 글리프 수와 일치) |
| 음길이 합제약 스냅 실패 | **0 / 58** |
| 결함성 경고 | **0**. `unsupported_glyph` 26건은 정보성(미반영 아티큘레이션·연주법) |
...
4. "합이 정확히 목표(4/4 → 4.0) 이면서 스냅 오차 총합이 최소" 인 legal 조합을 찾는다.
   legal 길이가 모두 0.125 의 배수이므로 정수 단위 DP 로 정확히 푼다.
```

> 처음에는 그리디 보정을 썼으나 2라운드 리뷰에서 지역최솟값 실패가 드러났다
> (`[4.0, 4.0]` 을 4.0 에 맞출 때 `[2.0, 2.0]` 이 있는데도 실패). DP 는 해가
> 존재하면 반드시 찾는다.
...
**결과: 58마디 전부 성공(실패 0), 마디1 = 8분음표 8개.** 빔·기둥·플래그·부점 글리프
탐지 코드 전체가 필요 없어진다.
...
## 부록 B: 2라운드 2way 리뷰 수정 목록 (구현 후)

Codex 가 **내가 만든 결함과 그 결함을 정답으로 고정한 테스트**를 잡았다. 12건 전부 반영했다.

| # | 심각도 | 발견 | 반영 |
|---|---|---|---|
| 1 | blocker | `_stroke_at` 이 시스템 y 를 무시해 스트로크 9개를 27개로 부풀림 | 타브 대역 제한. 테스트를 PDF 원본 글리프 수를 세는 독립 oracle 로 교체 |
| 2 | blocker | `ir_path` 가 입력 PDF 와 같으면 원본을 덮어쓰고 success 반환 | `.json` 강제 + 동일 경로 거부 |
| 3 | major | 음길이 그리디가 정확해 있어도 지역최솟값 실패 (`[4.0,4.0]`→`4.0`) | 정수 단위 DP 로 교체 (0.125 배수라 가능). 실패 테스트도 진짜 불가능한 경우로 |
| 4 | major | 스냅 실패한 프렛 beat 를 슬래시로 오인해 코드 보이싱으로 채움 | 좌표로 슬래시 출처 판정, 아니면 `empty_beat` 경고 |
| 5 | major | 모든 마디를 무조건 4/4 | SMuFL `timeSig` 검출, 4/4 아니면 미검증 경로 경고 |
| 6 | major | 아티큘레이션 44개·해머온/풀오프/슬라이드가 경고 없이 소실 | 종류별 집계 경고 (26건) |
| 7 | major | 두 자리 프렛 경고가 잉크 간격 음수일 때만 발화 | origin 간격 기준 추가 |
| 8 | major | `measure_bounds` 가 역방향 경계 `(50, 45)` 생성 | 좌측 끝보다 앞선 세로선 제거 |
| 9 | major | 테스트 기대값이 독립 oracle 이 아님 | 스트로크는 원본 글리프 수, 프렛은 숫자 수(294) 로 근거화 |
| 10 | minor | `_has_adjacent_letter` 가 O(G²) (3페이지 17만회) | baseline 인덱스 페이지당 1회 → 추출 53ms |
| 11 | minor | 광범위 `except Exception` 이 결함을 삼킴 | traceback 로깅 추가 |
| 12 | minor | `frozen` 인 `System` 의 list 가 변경 가능 | tuple 로 |

### 이번 라운드에서 확인된 것

- 2way 의 값은 "다른 모델이 다른 실수를 한다"가 아니라 **내가 만든 테스트를 의심해 준다**는 데 있었다.
  스트로크 27개와 그리디 실패는 둘 다 내 테스트가 정답으로 고정하고 있었다.
- 1라운드에서 이미 같은 계열의 버그(시스템 y 범위 미제한)를 고쳤는데, 구현 중 `_stroke_at` 에서
  똑같이 반복했다. 페이지에 시스템이 여러 개 쌓인 구조에서는 **x 만으로 찾는 모든 코드**가
  같은 버그를 낳는다.
